...
export function LoginForm() {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (loggedInUser.role === "admin" || loggedInUser.email.toLowerCase().includes("admin")) {
  ...
  const onDemo = async () => {
    ...
    try {
      ...
      setServerError(e instanceof Error ? e.message : "Demo login failed");
    } finally {
  ...
  const onDemoAdmin = async () => {
    ...
    try {
      ...
      setServerError(e instanceof Error ? e.message : "Demo admin login failed");
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <Button type="submit" fullWidth disabled={isSubmitting} className="shadow-md shadow-indigo-600/10 animate-fade-in">
